                            ...
                            @foreach($careerSteps as $index => $step)
                                <div class="relative pl-14">
                                    ...
                                    <div class="absolute left-0 top-0 transform translate-x-4">
                                        <div class="w-10 h-10 {{ $step->is_current ? 'bg-green-500' : 'bg-blue-500' }} rounded-full flex items-center justify-center shadow-md">
                                            <div class="w-8 h-8 bg-white rounded-full flex items-center justify-center">
                                                <div class="w-6 h-6 {{ $step->is_current ? 'bg-green-500' : 'bg-blue-500' }} rounded-full"></div>
                                            </div>
                                        </div>
                                    </div>
                                    
                                    <!-- Timeline Content -->
                                    <div class="bg-white p-5 rounded-lg shadow-md border-l-4 {{ $step->is_current ? 'border-green-500' : 'border-blue-500' }} hover:shadow-lg transition duration-300">
                                        <div class="flex justify-between items-start mb-2">
                                            <div>
                                                <h4 class="font-bold text-lg text-gray-800">{{ $step->title }}</h4>
                                                <p class="text-gray-600 text-sm">{{ $step->step_date->format('F d, Y') }}</p>
                                            </div>
                                            <div>
                                                @if($step->status)
                                        ...
                                        <div class="mt-3">
                                            @if($step->type)
                                                <div class="text-sm"><span class="font-medium text-gray-700">Type:</span> {{ $step->type }}</div>
                                            @endif
                                            
                                            @if($step->details)
                                                <div class="mt-2 text-sm text-gray-600">{{ $step->details }}</div>
                                            @endif
                                        </div>
                                        
                                        <!-- Admin actions -->
                                        <div class="mt-4 pt-3 border-t border-gray-100 flex space-x-4 justify-end">
                                            <button type="button" onclick="editStep({{ $step->id }})" class="text-xs flex items-center text-blue-600 hover:text-blue-800">
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                        ...
                        <div class="mt-4">
                            <form id="addStepForm" action="{{ route('employees.add-career-step', $employee->id) }}" method="POST" onsubmit="return validateStepDate()">
                                @csrf
                                <div class="space-y-4">
                                    <div>
